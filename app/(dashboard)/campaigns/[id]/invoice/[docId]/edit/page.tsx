...
import { getDocumentById, getLatestProformaForCampaign } from '@/lib/data/documents'
...
import type { InitialInvoiceDoc } from '../../new/invoice-form'
import type { SavedLineItem } from '@/lib/data/documents'

export async function generateMetadata() {
  return { title: 'Edit Invoice — Revflow' }
}
...
export default async function EditInvoicePage({
  ...
  const [doc, campaign, orgSettings, latestProforma] = await Promise.all([
  ...
  if (doc.status !== 'draft') redirect(`/campaigns/${id}/invoice/${docId}`)
  ...
  const initialDoc: InitialInvoiceDoc = {
    id: doc.id,
